...
class TicketItemTable extends DataTableComponent
{
    ...
    public function builder():EloquentBuilder
    {
        return Ticket::query()
            ...
            ->when($this->columnSearch['contact_number'] ?? null, fn($query, $contact_number) => 
        $query->whereHas('lead', fn($q) => $q->where('contact_number', 'like', '%' . $contact_number . '%'))
    )
    ->select('tickets.*', 'leads.contact_number'); // Ensure only ticket columns are selected if needed
