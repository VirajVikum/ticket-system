...
class TicketItemTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Contact", "leads.contact_number")
    ...
    public function builder():EloquentBuilder
    {
        return Ticket::query()
            ...
            ->when($this->columnSearch['contact_number'] ?? null, fn($query, $contact_number) => $query->where('tickets.lead->contact_number', 'like', '%' . $contact_number . '%'))
...
{
    return Ticket::query()
        ->select('tickets.*', 'leads.contact_number')
        ->join('leads', 'tickets.lead_id', '=', 'leads.id');
}
