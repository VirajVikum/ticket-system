...
class TicketItemTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            ->secondaryHeader(function() {
                return view('livewire.ticket-items.tables.cells.input-search', ['field' => 'lead.contact_number', 'columnSearch' => $this->columnSearch]);
            })
            ->footer(function($rows) {
                return '<strong>Name Footer</strong>';
            })
    ...
    public function builder():EloquentBuilder
    {
        return Ticket::query()
            ...
            ->when($this->columnSearch['lead.contact_number'] ?? null, fn($query, $contact_number) => $query->where('tickets.lead->contact_number', 'like', '%' . $contact_number . '%'))
        ->select('tickets.*'); // Ensure only ticket columns are selected if needed
