<div wire:poll.2000ms x-data="{
    progress: @entangle('progress'),
    rotation() {
        return 45 + (this.progress * 1.8);
    }
}" class="flex flex-col items-center" wire:poll.2000ms>
<div class="relative text-center">
    ...
    <div class="barOverflow relative overflow-hidden w-24 h-12">
        ...
        <div class="bar absolute top-0 left-0 w-24 h-24 rounded-full border-4 border-gray-200"
             :style="'transform: rotate(' + rotation() + 'deg); border-bottom-color: #0bf; border-right-color: #0bf;'">
        </div>
    </div>
    <!-- Percentage display -->
    <span class="absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center text-2xl font-bold text-gray-700" x-text="progress"></span>%
</div>
</div>
